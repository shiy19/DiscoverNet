...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v):
        ...
        output = torch.mul(attn.unsqueeze(-1), v.unsqueeze(1))
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, w):
        ...
        output, attn, log_attn = self.attention(q, k, v) # 1 x 256 x 64 x 64

        projected = torch.bmm(w, output.squeeze(0))
        
        output = self.dropout(self.fc(projected))
...
class MetricBase(nn.Module):
    def __init__(self, emb_model, args):
        ...
        self.metric_base1 = torch.nn.Parameter(torch.nn.init.kaiming_normal_(torch.zeros(1, args.dim_embed, args.dim_embed)))
        self.metric_base2 = torch.nn.Parameter(torch.nn.init.kaiming_normal_(torch.zeros(1, args.dim_embed, args.dim_embed)))
        self.set_func1 = nn.Sequential(nn.Linear(args.dim_embed, args.dim_embed*2), nn.ReLU(), nn.Linear(args.dim_embed*2, args.dim_embed))
        self.set_func2 = nn.Sequential(nn.Linear(args.dim_embed, args.dim_embed*2), nn.ReLU(), nn.Linear(args.dim_embed*2, args.dim_embed))
    ...
    def forward(self, x, y, z, c):
        ...
        whole_emb = whole_emb.permute([1,0,2])  # T x 3 x dim
        # x_emb, y_emb, z_emb = whole_emb.split(num_inst_x, dim=0)
        
        whole_emb_proto = torch.sum(self.set_func1(whole_emb), dim=1)
        whole_emb_proto = self.set_func2(whole_emb_proto) # T x dim
        # do transformation on (x,y) and (x,z)
        adapt_emb, _, _ = self.slf_attn(whole_emb_proto.unsqueeze(0), self.metric_base1, self.metric_base2, whole_emb)
